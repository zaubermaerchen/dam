...
`dam` は Unix パイプラインの起動ゲートです。v0.4.0 の CLI は `dam CONDITION [--or CONDITION]... [--buffer-size SIZE]`、単独指定の `dam --version` / `dam --describe`、および `-h` / `--help` を受け付けます。v0.4.0 では条件の構文が breaking change になり、release condition はすべて prefix 付きの positional argument で指定します。duration、datetime、signal、file の条件は OR で組み合わせ、1つの argument 内の ` && ` は AND group を表します。help、version、describe は stdin を読まずに stdout へ出力して終了します。
...
- `--describe` は単独指定時に schema version 1 の compact JSON を stdout へ出力して終了します。CLI、condition form、stdin/stdout/stderr、CLOSED → OPEN と EOF/error の state machine、空の `side_effects` を記述し、duration の `first-non-empty-read-completion` 開始点と `immediate-condition-satisfaction` (`0s`) を含め、signal capability は実行中の build に合わせます。`--describe` は stdin を読まず、release monitor、初回 file probe、pre-release buffer を開始しません。exact な help は優先し、それ以外の併記はエラーです。event-FD interface は記述しません。
- `--events-fd N` / `--events-fd=N` は専用 FD へ JSONL の `release-selected`、`stream-open` を OPEN 遷移中にこの順序で同期出力します。Unix では書き込み可能かつ既に nonblocking の pipe/FIFO/socket、Windows では書き込み可能と検証できる `PIPE_NOWAIT` pipe のみ受理します。無効または未対応の FD は stdin を読む前に設定エラー（終了コード 2）として拒否します。startup で release 済みなら stdin が blocked / empty でも両方を出力し、release 前の空 stdin EOF はどちらも出力しません。実行中に nonblocking mode を失った場合や write/short-write/EAGAIN 等の失敗時は観測を停止し、`events disabled: ...` の warning を一度だけベストエフォートで試み、data plane は継続します。FD とその mode は caller 所有とし、変更しません。
