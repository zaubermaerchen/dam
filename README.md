...
can also be opened by `SIGUSR1` or `SIGUSR2`. It also reports its version to
...
- Once configured, `SIGUSR1` and/or `SIGUSR2` remain intercepted and ignored
  ...
  opens the gate first. Unconfigured signals are not intercepted.
...
The current implementation provides a duration- and `SIGUSR1`/`SIGUSR2`-based,
one-way gate and version reporting. `SIGUSR1` and `SIGUSR2` are supported as
external release events. Repeated gate transitions, configurable buffer sizes,
spill-to-disk behavior, and an initially-open mode are not implemented. Signal
release is not available on Windows or other unsupported targets; those builds
reject the signal option explicitly while retaining duration and version
support.
