...
- `--events-fd N` / `--events-fd=N` は専用 FD へ JSONL の `release-selected`、`stream-open` を OPEN 遷移中にこの順序で同期出力します。startup で release 済みなら stdin が blocked / empty でも両方を出力し、release 前の空 stdin EOF はどちらも出力しません。setup/write/short-write/EAGAIN 等は `events disabled: ...` の warning を一度だけ出して観測を停止し、data plane は継続します。FD は caller 所有とし、Unix は元 flags、Windows は元 pipe mode を復元します。
